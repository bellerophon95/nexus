...
class SemanticCache:
    def __init__(
        ...
        self.redis_url = (
            redis_url
            or getattr(settings, "upstash_redis_rest_url", None)
            or os.getenv("UPSTASH_REDIS_REST_URL")
        )
        self.redis_token = (
            redis_token
            or getattr(settings, "upstash_redis_rest_token", None)
        ...
        if self.redis_url and self.redis_token:
            try:
                ...
                logger.info(
                    f"✅ Semantic Cache: Successfully connected to Upstash Redis ({self.redis_url[:20]}...)"
                )
            ...
            self.redis = None
            logger.warning(
                "⚠️ Semantic Cache: Redis credentials missing (UPSTASH_REDIS_REST_URL/TOKEN). Cache disabled."
            )
