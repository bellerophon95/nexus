...
from backend.ingestion.embedder import embed_chunk
from backend.ingestion.upserter import upsert_document, insert_chunks
...
def run_ingestion_pipeline(
    ...
    try:
        ...
        if progress_callback: progress_callback(15.0)
        ...
        if cleaned_doc.is_duplicate:
            logger.info(f"Deduplication triggered: Document already exists (Fingerprint: {cleaned_doc.fingerprint})")
            return {
                "status": "skipped", 
        ...
        # 3. Semantic Chunking
        if progress_callback: progress_callback(20.0)
        ...
        logger.info(f"Pipeline completed successfully in {time.time() - start_time:.2f}s. Document ID: {doc_id}")
