...
def enrich_chunk(text: str) -> dict[str, Any]:
    ...
    return {"entities": entities, "topics": topics, "key_phrases": topics[:5]}
...
def _extract_entities_from_doc(doc) -> list[dict[str, str]]:
    ...
    for ent in doc.ents:
        ...
        if ent_tuple not in seen_entities:
            entities.append({"text": ent_tuple[0], "label": ent_tuple[1]})
...
def enrich_chunks_batch(texts: list[str]) -> list[dict[str, Any]]:
    ...
    docs = nlp.pipe(
        texts, disable=["tagger", "parser", "attribute_ruler", "lemmatizer"], batch_size=16
    )
    ...
    for i, doc in enumerate(docs):
        ...
        results.append({"entities": entities, "topics": topics, "key_phrases": topics[:5]})
