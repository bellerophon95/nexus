...
async def run_worker_loop():
    ...
    while True:
        try:
            ...
            supabase.table("ingestion_chunks").update({"status": "processing"}).eq(
                "id", chunk_id
            ).execute()
            ...
            task_response = (
                ...
                .select("document_id, chunk_count, user_id")
                .eq("id", task_id)
                .execute()
            )
            ...
            # 5. Update Status
            supabase.table("ingestion_chunks").update({"status": "completed"}).eq(
                "id", chunk_id
            ).execute()
