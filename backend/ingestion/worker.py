...
async def run_worker_loop():
    ...
    while True:
        try:
            ...
            rpc_response = supabase.rpc("claim_ingestion_chunks", {"p_batch_size": BATCH_SIZE}).execute()
            ...
            for task_id, task_chunks in tasks_to_update.items():
                try:
                    ...
                    task_response = supabase.table("ingestion_tasks").select("document_id, chunk_count, user_id, metadata, is_personal").eq("id", task_id).execute()
                    ...
                    if not doc_id and meta:
                        import os
                        doc_type = os.path.splitext(meta.get("file_path", ""))[1][1:].lower() or "unknown"
                    ...
                    insert_chunks(doc_id, processed_batch, user_id=task_data["user_id"], is_personal=is_personal)

                    # Mark chunks as completed
                    chunk_ids = [c["id"] for c in task_chunks]
                    supabase.table("ingestion_chunks").update({"status": "completed"}).in_("id", chunk_ids).execute()
