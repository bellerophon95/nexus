...
def _reaper_monitor():
    ...
    while True:
        try:
            ...
            if stale_tasks:
                logger.warning(f"Reaper: Found {len(stale_tasks)} stalled tasks. Attempting Recovery.")
                for task in stale_tasks:
                    ...
                    chunk_res = supabase.table("ingestion_chunks").select("id").eq("task_id", task_id).eq("status", "processing").execute()
                    processing_chunks = chunk_res.data or []
                    
                    if processing_chunks:
                        logger.warning(f"Reaper: Task {task_id} ({filename}) is orphaned. Recycling {len(processing_chunks)} chunks to 'pending'.")
                        ...
                        logger.error(f"Reaper: Terminal failure forced for task {task_id} ({filename})")
                        supabase.table("ingestion_tasks").update({
                            "status": "error",
